...
async function sincronizarAssets(audioData, imagenData, sessionId) {
  try {
    await logAndNotify(sessionId, '🔄 SINCRONIZANDO ASSETS - Crítico para Hedra');
    await logAndNotify(sessionId, `📸 Imagen Asset ID: ${imagenData.imageAssetId}`);
    await logAndNotify(sessionId, `🔊 Audio Asset ID: ${audioData.audioAssetId}`);
    ...
    await logAndNotify(sessionId, '⏳ Esperando procesamiento de Hedra (30s)...');
    ...
    await logAndNotify(sessionId, `❌ Error en sincronización: ${error.message}`);
    throw error;
  }
}
...
async function crearVideo(assetsSync, sessionId) {
  try {
    ...
    await logAndNotify(sessionId, `🎬 Video iniciado en Hedra: ${generationId}`);
    ...
    await logAndNotify(sessionId, `❌ Error creando video: ${error.response?.status || error.message}`);
...
async function verificarEstadoVideo(generationId, sessionId) {
  try {
    await logAndNotify(sessionId, `🔍 Verificando estado del video: ${generationId}`, false); // Solo console, no Telegram
    ...
    await logAndNotify(sessionId, `❌ Error verificando estado: ${error.message}`);
    throw error;
  }
}
...
async function descargarVideo(videoUrl, sessionId) {
  try {
    ...
    await logAndNotify(sessionId, `📊 Video descargado: ${response.data.byteLength} bytes`);
    ...
    await logAndNotify(sessionId, `✅ Video guardado correctamente: ${videoPath} (${videoSize} MB)`);
    ...
    await logAndNotify(sessionId, `❌ Error descargando video: ${error.message}`);
    throw error;
  }
}
...
async function procesarVideoCompleto(audioData, imagenData, sessionId) {
  try {
    ...
    const assetsSync = await sincronizarAssets(audioData, imagenData, sessionId);
    ...
    await logAndNotify(sessionId, '⏳ Esperando generación de video (5 minutos)...');
    ...
    while (!videoListo && intentos < 15) {
      const estado = await verificarEstadoVideo(videoGeneration.generationId, sessionId);
      ...
      if (estado.ready && estado.url) {
        ...
        await logAndNotify(sessionId, `⏳ Video aún procesando... intento ${intentos + 1}/15`);
    ...
    if (!videoListo) {
      await logAndNotify(sessionId, '⚠️ Timeout en verificaciones, intentando descarga directa...');
      
      // EMPUJÓN: Intentar obtener el video una vez más
      const estadoFinal = await verificarEstadoVideo(videoGeneration.generationId, sessionId);
      if (estadoFinal.url) {
        ...
        await logAndNotify(sessionId, `❌ Video no completado tras ${intentos} intentos.`);
        await logAndNotify(sessionId, `🆔 ID para rescate manual: ${videoGeneration.generationId}`);
        await logAndNotify(sessionId, `💡 Usa: node rescue-video.js con este ID`);
    ...
    await logAndNotify(sessionId, '🎉 VIDEO COMPLETADO EXITOSAMENTE');
    ...
    await logAndNotify(sessionId, `❌ Error en proceso completo: ${error.message}`);
    throw error;
  }
}
